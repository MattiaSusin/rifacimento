package mattiasusin.rifacimento.security;
...
@Configuration
@EnableWebSecurity
@EnableMethodSecurity // Se voglio poter dichiarare le regole di autorizzazione direttamente sui singoli endpoint, allora questa annotazione è OBBLIGATORIA!!!
public class ConfigurationSecurity {
    @Bean
    SecurityFilterChain securityFilterChain(HttpSecurity httpSecurity) throws Exception {

        // TODO Per poter configurare Security devo crearmi un Bean apposito che mi consenta di:
        // 1 --> disabilitare dei comportamenti di default non graditi
        httpSecurity.formLogin(http -> http.disable()); // Non voglio il form di login (avremo React per quello)
        httpSecurity.csrf(http -> http.disable()); // Non voglio la protezione da attacchi CSRF (per le nostre app non è necessaria, anzi andrebbe a complicare anche il codice lato FE)
        ...
        // 2 --> customizzare il comportamento di alcuni filtri di sicurezza

        // 3 -->  aggiungere ulteriori filtri personalizzati
